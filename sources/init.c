...
/*   Updated: 2023/08/14 01:04:48 by yodahani         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	init_game(t_test *test)
{
	...
	test->mini->addr = mlx_get_data_addr(test->mini->ptr, &test->mini->bits_per_pixel,
...
void	init_texture_fill(t_test *game,t_img *txt)
{
	...
	path = txt->ptr;
	txt->ptr = mlx_xpm_file_to_image(game->mlx_ptr, path, &txt->width, &txt->heigth);
	if (!txt->ptr)
		printerror("Error while loading texture", NULL);
	txt->addr = mlx_get_data_addr(txt->ptr, &txt->bits_per_pixel, &txt->line_length, &txt->endian);
	free(path);
}
